  ...
  test("renders the failing tool input, captured error text, and a ✗ glyph", async () => {
    vi.spyOn(api, "getSessionErrors").mockResolvedValue({
      ...
      errors: [{
        ...
        before: [{ role: "user", text: "fix the bug", isError: false }],
        after: [],
        index: 1,
      }],
    });
  ...
  test("the expand control grows the visible context window", async () => {
    vi.spyOn(api, "getSessionErrors").mockResolvedValue({
      ...
      errors: [{
        ...
        errorText: "boom",
        before: [
          { role: "user", text: "CTX_B1", isError: false },
          { role: "assistant", text: "CTX_B2", isError: false },
          { role: "assistant", text: "CTX_B3", isError: false },
        ],
        after: [
          { role: "assistant", text: "CTX_A1", isError: false },
          { role: "assistant", text: "CTX_A2", isError: false },
        ],
        index: 3,
      }],
    });

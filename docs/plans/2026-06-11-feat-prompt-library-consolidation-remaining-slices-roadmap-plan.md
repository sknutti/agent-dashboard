...
- **Shipped-status (updated 2026-06-12):** Slices **3 (working copy/editor)**, **4 (versioning/publishing)**, **5 (target overlays)**, **6 (metadata editing)**, **7 (reimport-from-drift)**, **9 (search)**, **L (primitive lifecycle)**, and **2 (bootstrap discovery wizard, incl. the Reconcile/`forget` view)** are **DONE** — see the per-slice headers below. **The only remaining feature slice is 8 (git remote sync)** — the sole network + secrets break, held for last; 10b (URL import) rides its network posture. 10's remaining native-affordance pieces are decision-logged. Slice 6's deepened plan is [2026-06-12-...-metadata-editing-slice-plan.md](2026-06-12-feat-prompt-library-metadata-editing-slice-plan.md); Slice 7's is [2026-06-12-...-reimport-from-drift-slice-plan.md](2026-06-12-feat-prompt-library-reimport-from-drift-slice-plan.md); Slice 9's is [2026-06-12-...-search-slice-plan.md](2026-06-12-feat-prompt-library-search-slice-plan.md); Slice L's is [2026-06-12-...-lifecycle-slice-plan.md](2026-06-12-feat-prompt-library-lifecycle-slice-plan.md). Remaining unbuilt: 8, 2, 10 (10a's typed-path import-from-path landed with L; `forget`'s UI awaits Slice 2's Reconcile view).
...
[shipped] read-only (#4) ──> [shipped] install/drift (#5)
   ...
   [shipped] 2.  Bootstrap discovery wizard + Reconcile/forget (depends on lifecycle + reimport)
```
...
## Slice 3: Working copy / editor — ✅ SHIPPED (PR #6, `66905f6`)

> **Status:** Done and merged. Deepened plan: [2026-06-11-...-working-copy-editor-slice-plan.md](2026-06-11-feat-prompt-library-working-copy-editor-slice-plan.md). Landed as Phases 1–4 (`4d80ea9`→`a202d6b`) + post-review fixes (`7fcbee7` save-race/createFile lock, `bf9ab09` case-insensitive primary-filename security guard). Seams now live: 7 bridge dispatch arms (`main.rs`), `WorkingFileEntry`/`WorkingFileBytes` models + parsers, `/api/library` working-file routes, `WorkingFileEditor.svelte`. The objective and detail below are retained as the historical record.
...
## Slice 4: Versioning / publishing — ✅ SHIPPED (`39933b8`)

> **Status:** Done and merged. Deepened plan: [2026-06-12-...-versioning-publishing-slice-plan.md](2026-06-12-feat-prompt-library-versioning-publishing-slice-plan.md). The publish-atomicity decision (snapshot-then-commit, not atomic across the commit step) and the commit-on-write posture were settled here — **this is the decision Slice L's git-commit half consumes.** The objective and decisions below are retained as the historical record.
...
## Slice 5: Target overlays — ✅ SHIPPED

> **Status:** Done. Deepened plan: [2026-06-12-...-target-overlays-slice-plan.md](2026-06-12-feat-prompt-library-target-overlays-slice-plan.md) (all acceptance criteria checked). Landed as 3 phases on `feat/library-target-overlays`: bridge commands (4 sync dispatch arms, zero new error arms — `TargetNotAllowed`→422 already mapped), `/api/library/targets/:target` + `/overlays` routes (writes get WRITE_TIMEOUT + SIGKILL, no ledger mutex — gitignored `working/targets/`, no commit), and `TargetOverlayPane.svelte` (per-`allowed_targets` tab strip, `overlayCue` cyan ◆ vs default ○, Decision-3 reinstall/drift note). Scoped to **primary-file overlays only** (the reference has no per-ref overlay command). The objective and risks below are retained as the planning frame.
...
## Slice 7: Reimport-from-drift — ✅ SHIPPED (2026-06-12)

> **Status:** Done. Deepened plan: [2026-06-12-...-reimport-from-drift-slice-plan.md](2026-06-12-feat-prompt-library-reimport-from-drift-slice-plan.md). Landed as 3 phases on `feat/library-reimport-from-drift`: bridge `reimport_install` (async, commits on `Reimported` only — a deliberate DIVERGENCE from the reference, which never commits reimport), `POST …/reimport` route + `ReimportResult` model (takes `withWriteLock` — the one divergence from publish, since core re-baselines `installs.json`; all five variants ride 200 as data), and the three-action drift row (Acknowledge / Reinstall / Reimport) + `working_copy_dirty` discard-confirm + `broken_source` fix-sheet flows. The objective/risks below are retained as the planning frame.
...
## Slice 9: Search — ✅ SHIPPED (2026-06-12)

> **Status:** Done on branch `feat/library-search`. Deepened plan: [2026-06-12-...-search-slice-plan.md](2026-06-12-feat-prompt-library-search-slice-plan.md). Pure wiring across 4 seams — core (`find.rs`, 10 tests) was already imported. One sync read bridge arm `cmd_find_in_library` (zero new error arms — `Io`→`library_unreadable` already mapped; secrets-free, no mutex, no commit), a `SearchResult` model + `parseSearchResults`, a `GET /api/library/search?q=` read route (no write lock, 10s read timeout; `library_unconfigured`→**409**), and a debounced content-search box in `Library.svelte` (distinct from the name filter; `setTimeout`-in-handler debounce, no useEffect; result rows reuse `selectPrimitive(selectionKey(...))`). **A1 (cost) discharged:** ~50–80ms full spawn against the real 117-primitive library vs the 10s timeout — no index needed (open-question #4 closed). Gates: cargo 648, scripts 204, Library vitest 44, svelte-check 0. The objective/risks below are retained as the planning frame.
...
## Slice 2: Bootstrap discovery wizard — ✅ SHIPPED (2026-06-12)

> **Status:** Done on `feat/library-bootstrap-wizard`. Deepened plan: [2026-06-12-...-bootstrap-discovery-wizard-slice-plan.md](2026-06-12-feat-prompt-library-bootstrap-discovery-wizard-slice-plan.md). Three phases — bridge dispatch + 2 config paths (`cff8b5a`), TS routes/models/parsers (`25f0440`), the 4-step wizard + Reconcile/forget UI (`094e567`). **The roadmap's "hard part" (progress-streaming) dissolved at source:** `ScanProgress` is 3 fixed coarse stages + Done over a sub-second scan, so the Tauri `Channel` was **dropped entirely** (a no-op `on_progress`, single request→single-response, client-side stage label) — not even option (b)'s session-poll was needed. `derive_plan` runs server-side inside `cmd_bootstrap_scan` → `{cross_referenced, plan}` in one envelope. Execute commits only when `created+reimported>0` (the reimport gating divergence). **`forget` got its Reconcile-view home in this slice** (Scott's call), deriving orphans client-side from the existing driftBatch+primitives reads — no new bridge command. One source-driven divergence: core uses a single `timestamp` for both the backup filename and `created_at`, so the bridge matches the reference's filesafe-for-both rather than the deepened plan's split. **10a's typed-path import was already done in Slice L.** The objective/risks below are retained as the planning frame.
...
- **10a — Folder/path entry (replaces `pick_library_path:167` and `import_primitive_from_path`'s native picker).** The read-only slice already chose **`config/library.yaml` + a typed path** over a native picker (ADR-0007). The remaining gap: import-from-path (Slice L) and any "open another library" flow need a **web path-entry UX** — a validated text input with existence/`.prompt-library`-marker checks server-side, not an OS dialog. **Decision posture:** typed path + server validation, consistent with the existing config approach. Lands with Slice L.
  - **Library lifecycle (`init_library:188`, `get_library_path:231`, reference `c20f71a` switch-folder Settings dialog) — CONSCIOUSLY DROPPED in favor of config (Scott, 2026-06-12).** A source-level parity audit (all 61 reference commands + a byte-diff of `crates/{core,git,secrets}`) confirmed the consolidation is feature-complete for **operating an existing library**, with the crates *ahead* of the reference (the only two differing files — `core/url_import.rs`'s `Policy::none()` SSRF fix, `git/git_ops.rs`'s `set_origin_remote`— are dashboard additions). The single top-level gap is **library lifecycle itself**: creating a brand-new empty library (`init_library` — the core fn `library_init::init_library` is present and tested in this repo but has **no bridge command/route/UI**) and switching the active library in-app. **Decision: `config/library.yaml` is the intended door for both** — point at or create a library by editing the config (and restart to switch), no in-app create/switch affordance. `get_library_path` is already covered by `library_status`. This is the recorded web answer for these three commands, which the roadmap's acceptance criteria otherwise never assigned to a slice.
...
**All feature slices have shipped.** Slices 3, 4, 5, 6, 7, 9, **L**, **2 (bootstrap + Reconcile/`forget`)**, and now **8 (git remote sync)** are done. Slice 8 — the sole network + secrets break — landed across Phases 0–4 + 3.5 on `feat/library-git-remote-sync` (deepened plan: [2026-06-12-...-git-remote-sync-slice-plan.md](2026-06-12-feat-prompt-library-git-remote-sync-slice-plan.md)): the keychain spike (verified headless one-shot reads), the secrets break (`prompt-library-secrets` linked, ADR-noted), the egress break (push/pull/scan-gate + the stateful conflict family, **security-reviewed clean** with the one Low finding fixed), the TS routes + redaction tripwire, git-origin wiring from `configure_remote` (a deliberate divergence — the reference only persists), and the CVD-safe sync panel + conflict resolver UI. Gates: cargo `--workspace` 698, scripts 429, svelte-check 442/0, ui vitest 194. Slice 8 merged via **PR #14**.

**10b (URL import) — MERGED (PR #15).** The immediate fast-follow shipped across 3 phases on `feat/library-url-import` (deepened plan: [2026-06-12-...-url-import-slice-10b-plan.md](2026-06-12-feat-prompt-library-url-import-slice-10b-plan.md)): the second network egress (reqwest, riding Slice 8's posture) — `fetch_primitive_from_url` + the optional `imported` create seed, the `/api/library/import/fetch` route + a dedicated 429, and the "Import from URL" create-form affordance. Two settled decisions: **redirects disabled in core** (`Policy::none()`, closing the SSRF redirect gap) and the **dedicated 429**. SSRF + m4-URL-leak tripwire-tested; not secrets-bearing. Gates: cargo 704, scripts 438, svelte-check 442/0, ui vitest 199.

**The consolidation is feature-complete.** Both Slice 8 and 10b await only **browser QA** (real push/pull + a real GitHub fetch — needs Scott) as post-merge validation. The remaining Slice 10 native-affordance gaps (reveal-in-Finder, iCloud check, recents) are decision-logged as droppable/redesigned; nothing else is outstanding.

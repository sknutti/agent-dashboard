...
## Amendment (2026-06-11): write-flow slice — install/drift scope and drift delivery

The first write-flow slice (the one this ADR deferred Drift and Install records to) is scoped and these decisions are recorded here because two of them are architectural and hard to reverse.

- **Slice command set: the minimal install/drift loop.** `install`, `uninstall`, `scan_drift`, `acknowledge_drift`, and `list_installs_for_primitive`. This closes a complete loop — install a Primitive to Targets, see where it is installed, detect Drift, reinstall or acknowledge, uninstall. `scan_library_drift` (the launch-time "the Primitive's library source directory was deleted" reconcile — a `MissingPrimitive`, a distinct concept from install Drift) and `subscribe_drift` are deferred. Git sync (pull/push/conflict), publish/versioning, import, and PAT/secrets stay out, so the install bridge keeps the read path's network-free and secrets-free invariants.

- **Migration is an explicit one-click import, not automatic.** The standalone app's `installs.json` (verified on this machine: 119 records — 49 skills, 64 agents, 6 commands across Claude/Codex/Pi) is imported into `DATA_DIR/installs.json` by an explicit user action in the Library route, not a silent read of `~/Library/Application Support/com.sknutti.promptlibrary/installs.json` on first run. The import is idempotent (refuses to clobber an existing `DATA_DIR/installs.json`) and `format_version`-guarded. Migrated records are immediately Drift-scannable because the dashboard's `InstallPaths` root (`CC_LIBRARY_HOME`) defaults to the same user home the standalone app installed against — same machine, same install paths. Rejected: automatic-on-first-run (a silent `~/Library` read is more surprising than a one-click action and gives no explicit "I am switching over now" moment).

- **Drift is delivered by pull-polling a batch scan, not a push watcher.** A single bridge command loads `installs.json`, scans every recorded `(kind, name)` via `core_scan_drift`, and returns all `DriftReport`s in one process spawn; that payload feeds both the explorer Drift badges and the selected Primitive's detail. It rides the existing 30s dashboard poll. `subscribe_drift` — the reference core's live filesystem-watch event API — is **not ported to the web app**. Rationale: even the standalone Tauri app, which could use a real watcher, polls Drift instead (`startDriftPolling`, a focus-gated interval rescan); a localhost HTTP dashboard has no reason to build a push channel the desktop app itself declined to use, and the one-shot command bridge has no event transport. A future reader finding `subscribe_drift` in core but not in the bridge should read this as deliberate, not an omission. Colorblind-safe status cues (per ADR-0007 / the read-only slice) apply to the returned Drift states.
